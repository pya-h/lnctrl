...
const constructStep1 = (sI_A, landa, poles) =>
    ...
    landa.toString() + " \\\\ \\\\ " +
    poles
        .map((pi, i) => "s_{" + (i + 1).toString() + "} = " + (pi instanceof Algebra ? pi.toString() : calculus.strictPrecisionFormat(calculus.round(pi))))
        .join(" , ") +
    " \\end{cases}$$";
...
const Fi_ij = (delta, k1, k2, p1, p2 = p1) => {
    ...
    if (delta !== 0) {
        fi_s = new Algebra(k1, {
            ...
            type: "frac",
            b: [1, p1],
        }).add(new Algebra(k2, { symbol: "s", type: "frac", b: [1, p2] }));
        fi_t = new Algebra(k1, { type: "exp", b: p1 instanceof Algebra ? p1.negation() : -p1 }).add(
            new Algebra(k2, { type: "exp", b: p2 instanceof Algebra ? p2.negation() : -p2 })
        );
    } else {
        fi_s = new Algebra(k1, {
            symbol: "s",
        ...
        fi_t = new Algebra(k1, { type: "exp", b: p1 instanceof Algebra ? p1.negation() : -p1 }).add(
            new Algebra(k2, {
                type: "exp",
                b: p1 instanceof Algebra ? p1.negation() : -p1,
                dot: new Algebra([1, 0], { symbol: "t", type: "poly" }),
            })
        );
    }
    return [fi_s, fi_t];
...
const SpaceStateEquationSolveBox = ({ A, x_0, C }) => {
    ...
    useEffect(() => {
        ...
        $sI_A(
            new Matrix([
                [
                    new Algebra([1, -A[0][0]], { symbol: "s" }),
                    new Algebra([-A[0][1]], { symbol: "s" }),
                ],
                [
                    new Algebra([-A[1][0]], { symbol: "s" }),
                    new Algebra([1, -A[1][1]], { symbol: "s" }),
                ],
            ])
        );
        $landa(
            new Algebra([1, -(A[0][0] + A[1][1]), matrixA.det()], {
                symbol: "s",
            })
        );
