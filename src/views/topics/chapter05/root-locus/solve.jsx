...
const determine = ({ zeroRows, signChanges }) => {
    ...
    if (polesOnRightHalf) {
        let str = `This system has ${polesOnRightHalf} Pole to the right of the imaginary axis `;
        if (couplePolesOnImgAxis) {
            ...
            str += `(${asymmetricalPoles} asymmetric - ${symmetricalPoles} symmetric) `;
        }
    ...
    if (couplePolesOnImgAxis) {
        systemProps.push(
            `This system has ${couplePolesOnImgAxis} The pole pair lies on the imaginary axis.`
        );
    }
    if (criticallyStable)
        systemProps.push("As a result, the system is marginally stable (oscillatory).");
    else if (stable) {
        systemProps.push(
            "All poles of this system are located to the left of the imaginary axis."
        );
        systemProps.push("Therefore the system is stable.");
    } else systemProps.push("Therefore the system is unstable.");
    return systemProps;
};
const RootLocusSolveBox = ({ a_i, epsilon, showEpsilonBar }) => {
    ...
    useEffect(() => {
        ...
        if (n > 0) {
            ...
            if (systemProps.length) st.push(...systemProps);
                ...
                st.push(
                    "A problem has occurred in determining the status of the poles and the stability of the system."
                );
...
export default RootLocusSolveBox;
